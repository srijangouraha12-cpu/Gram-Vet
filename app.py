...
def serve_media(filename):
    user_uploaded = r'C:\Users\amitp\.gemini\antigravity\brain\428b30d2-37ce-4ff4-882e-00a8526c0203\.user_uploaded'
    brain_dir = r'C:\Users\amitp\.gemini\antigravity\brain\428b30d2-37ce-4ff4-882e-00a8526c0203'
    if os.path.exists(os.path.join(user_uploaded, filename)):
        return send_from_directory(user_uploaded, filename)
    return send_from_directory(brain_dir, filename)
...
def mongo_upsert(collection, document):
    ...
    try:
        ...
        print(f"[GramVet] MongoDB sync skipped for {collection}: {e}", flush=True)
...
def assigned_vet_by_gps(c, farmer_lat, farmer_lon):
    """Find the best vet for a farmer:
    1. Any vet whose GPS is within 5km of farmer
    2. Otherwise the closest vet by distance
    ...
    if in_range:
        ...
        return in_range[0][1]
    # Fallback: closest vet regardless of range
    closest = None
...
def _reassign_farmers_to_vet(c, new_vet_id, vet_lat, vet_lon):
    """When a new vet registers, reassign open cases of farmers within 5km to this vet."""
    all_animals = c.execute("""
        SELECT a.id animal_id, a.farmer_id, a.village_id,
               COALESCE(a.latitude, v.latitude) lat,
               COALESCE(a.longitude, v.longitude) lon
        FROM animals a
        LEFT JOIN villages v ON v.id = a.village_id
    """).fetchall()
    reassigned_villages = set()
    for a in all_animals:
        if a["lat"] is None or a["lon"] is None:
            continue
        if haversine(vet_lat, vet_lon, a["lat"], a["lon"]) <= RADIUS_KM:
            village_id = a["village_id"]
            if village_id and village_id not in reassigned_villages:
                c.execute("INSERT OR IGNORE INTO vet_villages(vet_id,village_id) VALUES(?,?)", (new_vet_id, village_id))
                reassigned_villages.add(village_id)
            # Reassign all open cases for this animal to new vet
            c.execute("UPDATE cases SET vet_id=? WHERE animal_id=? AND status!='CLOSED'", (new_vet_id, a["animal_id"]))
...
def call_disease_model(a, symptoms, weather=None, c=None, report_inputs=None):
    ...
    try:
        ...
        herd_size=report_inputs.get("herd_size")
        waterbody_km=report_inputs.get("dist_waterbody_km")
        payload={
          ...
          "Herd_Size":max(1,int(herd_size or v or 1)), **flags,
          ...
          "Dist_Nearest_Outbreak_km":5.0,"Dist_Waterbody_km":float(waterbody_km if waterbody_km is not None else 1.2),
        ...
        result["input_summary"]={"age_months":payload["Age_Months"],"sex":payload["Sex"],"herd_size":payload["Herd_Size"],"dist_waterbody_km":payload["Dist_Waterbody_km"],"recent_treatment":payload["Recent_Treatment"],"vaccinations":flags,"vaccine_months":report_inputs.get("vaccine_months",{}),"vaccine_dates":report_inputs.get("vaccine_dates",{}),"weather":{k:payload[k] for k in ("Temperature_C","Humidity_Percent","Rainfall_mm","Season")}}
...
def signup():
    ...
    try:
        ...
        # Store GPS on user record (important for vets)
        ...
        # If a new vet just registered: reassign farmers in 5km range to this vet
...
def report_case(u,c):
    ...
    # Capture live GPS if provided by the user's device when reporting
    raw_lat = d.get("latitude")
    raw_lon = d.get("longitude")
    try:
        report_lat = float(raw_lat) if raw_lat is not None else None
        report_lon = float(raw_lon) if raw_lon is not None else None
    except (ValueError, TypeError):
        report_lat, report_lon = None, None

    if report_lat is not None and report_lon is not None:
        # Update animal's coordinates with exact GPS from this report
        c.execute("UPDATE animals SET latitude=?, longitude=? WHERE id=?", (report_lat, report_lon, a["id"]))
        # Set village coordinates if not set, but KEEP village name as entered by user
        c.execute("UPDATE villages SET latitude=?, longitude=? WHERE id=? AND (latitude IS NULL OR longitude IS NULL)", (report_lat, report_lon, a["village_id"]))
    ...
    lat,lon,loc_source=resolve_location(c,a,report_lat,report_lon)
    ...
    current_disease = top.get("disease")
    
    outbreak_alert = False
    outbreak_level = None  # "MID" or "HIGH"
    affected_farmers = set()
    cluster_count = 1
    
    if current_disease and lat is not None and lon is not None:
        # Count same disease cases within vet's 5km in last 7 days
        vet_for_farmer = assigned_vet_by_gps(c, lat, lon)
        vet_center_lat = (vet_for_farmer["latitude"] if vet_for_farmer and vet_for_farmer["latitude"] else lat)
        vet_center_lon = (vet_for_farmer["longitude"] if vet_for_farmer and vet_for_farmer["longitude"] else lon)

        recent_reports = c.execute("""
            SELECT hr.id as report_id, hr.case_id, hr.animal_id, a.farmer_id, hr.ai_prediction_json, a.latitude, a.longitude, v.latitude as v_lat, v.longitude as v_lon
            FROM health_reports hr
            JOIN animals a ON a.id = hr.animal_id
            LEFT JOIN villages v ON v.id = a.village_id
            WHERE hr.reported_at >= date('now', '-7 days')
        """).fetchall()
        
        cluster_reports = []
        for rr in recent_reports:
            r_pred = json.loads(rr["ai_prediction_json"] or "{}")
            r_top = (r_pred.get("predictions") or [{}])[0] if isinstance(r_pred,dict) else {}
            if r_top.get("disease") == current_disease:
                r_lat = rr["latitude"] if rr["latitude"] is not None else rr["v_lat"]
                r_lon = rr["longitude"] if rr["longitude"] is not None else rr["v_lon"]
                if r_lat is not None and r_lon is not None and haversine(vet_center_lat, vet_center_lon, r_lat, r_lon) <= RADIUS_KM:
                    cluster_count += 1
                    affected_farmers.add(rr["farmer_id"])
                    cluster_reports.append(rr)
        
        if cluster_count >= 3:
            outbreak_alert = True
            outbreak_level = "HIGH" if cluster_count > 5 else "MID"
            prob_val = 0.99 if cluster_count > 5 else 0.65
            prediction["outbreak_probability"] = prob_val
            prediction["outbreak_status"] = outbreak_level
            prediction["escalation_probability"] = prob_val
            prediction["outbreak_level"] = outbreak_level
            level_label = "CRITICAL" if outbreak_level == "HIGH" else "MID-RISK"
            prediction["message"] = f"{level_label} OUTBREAK ALERT: {cluster_count} animals within 5km showing signs of {current_disease}!"
            
            for rr in cluster_reports:
                r_pred = json.loads(rr["ai_prediction_json"] or "{}")
                r_pred["outbreak_probability"] = prob_val
                r_pred["outbreak_status"] = outbreak_level
                r_pred["outbreak_level"] = outbreak_level
                r_pred["escalation_probability"] = prob_val
                r_pred["message"] = prediction["message"]
                c.execute("UPDATE health_reports SET ai_prediction_json=? WHERE id=?", (json.dumps(r_pred), rr["report_id"]))
                
            nearby_animals = c.execute("""
                SELECT a.farmer_id, a.latitude, a.longitude, v.latitude as v_lat, v.longitude as v_lon
                FROM animals a
                LEFT JOIN villages v ON a.village_id = v.id
            """).fetchall()
            for na in nearby_animals:
                n_lat = na["latitude"] if na["latitude"] is not None else na["v_lat"]
                n_lon = na["longitude"] if na["longitude"] is not None else na["v_lon"]
                if n_lat is not None and n_lon is not None and haversine(vet_center_lat, vet_center_lon, n_lat, n_lon) <= RADIUS_KM:
                    affected_farmers.add(na["farmer_id"])
                
            for f_id in affected_farmers:
                if f_id != u["id"]:
                    create_notification(c, f_id, f"{level_label} Outbreak Advisory", 
                        f"Possible {'HIGH RISK' if outbreak_level=='HIGH' else 'moderate risk'} outbreak of {current_disease} detected within 5km. Monitor your cattle closely.", 
                        "CRITICAL" if outbreak_level=="HIGH" else "HIGH", "ADVISORY", None, a["village_id"])
    ...
    if vet:
        ...
        if outbreak_alert:
            sev = "CRITICAL" if outbreak_level=="HIGH" else "HIGH"
            level_label = "CRITICAL" if outbreak_level=="HIGH" else "MID-RISK"
            create_notification(c,vet["id"],f"{level_label} OUTBREAK ALERT",
                                f"{cluster_count} animals within 5km showing signs of {disease}. {'Immediate' if outbreak_level=='HIGH' else 'Urgent'} action required!",
                                sev, "OUTBREAK", case_id, a["village_id"])
            create_notification(c,u["id"],"Outbreak Advisory", f"Possible {'HIGH RISK' if outbreak_level=='HIGH' else 'moderate'} outbreak of {disease} in your area. Vet alerted.", sev, "ADVISORY", case_id, a["village_id"])
            
            govs = c.execute("SELECT id FROM users WHERE role='government'").fetchall()
            for g in govs:
                create_notification(c, g["id"], f"{level_label} OUTBREAK ALERT",
                                    f"AI alert ({cluster_count} animals, 5km): {disease} — {'HIGH' if outbreak_level=='HIGH' else 'MODERATE'} risk.",
                                    sev, "OUTBREAK", case_id, a["village_id"])
        else:
...
def list_cases(u,c):
    ...
    where=[]
    sql="""
      SELECT cs.*,a.name animal_name,a.tag animal_tag,a.species,a.farmer_id,a.village_id,
...
def vet_vaccine(u,c,case_id):
    cs=c.execute("SELECT * FROM cases WHERE id=? AND vet_id=?",(case_id,u["id"])).fetchone()
    if not cs:return jsonify(error="Case not found or not assigned to you."),404
    d=request.json or {}; name=(d.get("vaccine_name") or "").strip().upper(); unit=(d.get("unit") or "dose").strip()
    if name not in ("FMD", "HS", "LSD", "BQ"):
    ...
    dispatched = c.execute("SELECT COALESCE(SUM(quantity_change),0) FROM inventory_transactions WHERE vet_id=? AND reason='Govt Dispatch' AND upper(resource_name)=?", (u["id"], name)).fetchone()[0]
    used = c.execute("SELECT COALESCE(SUM(dose),0) FROM vaccinations WHERE vet_id=? AND upper(vaccine_name)=?", (u["id"], name)).fetchone()[0]
    
    resources_left = 20.0 + dispatched - used
    
    if resources_left < dose:
        return jsonify(error=f"Insufficient stock for {name}. You have {resources_left:g} {unit}s left. Please request more from government."),400
    ...
    new_stock = resources_left - dose
    
    if new_stock < 5:
        c.execute("INSERT INTO notifications(user_id,title,message,severity,kind,created_at) VALUES(?,?,?,?,?,?)",
                  (u["id"], f"Low Stock Alert: {name}", f"Your stock for {name} has fallen to {new_stock:g}. Auto-requesting 50 doses from government.", "CRITICAL", "LOW_STOCK", now()))
        existing = c.execute("SELECT 1 FROM resource_requests WHERE vet_id=? AND status='PENDING' AND resource_json LIKE ?", (u["id"], f"%{name}%")).fetchone()
        if not existing:
            auto_req = json.dumps([{"name": name, "quantity": 50, "unit": "dose"}])
            a = c.execute("SELECT village_id FROM animals WHERE id=?", (cs["animal_id"],)).fetchone()
            c.execute("""
              INSERT INTO resource_requests(vet_id,village_id,case_count,resource_json,reason,created_at)
              VALUES(?,?,?,?,?,?)
            """,(u["id"], a["village_id"], 1, auto_req, f"Auto-request: Stock fell below 5 doses.", now()))
            for g in c.execute("SELECT id FROM users WHERE role='government'").fetchall():
                create_notification(c,g["id"],"Veterinary resource request",
                    f"Vet {u['name']} automatically requested {name} (stock low).",
                    "HIGH","RESOURCE",None,a["village_id"])
...
def vet_overview(u,c):
    ...
    dispatched = c.execute("SELECT COALESCE(SUM(quantity_change),0) FROM inventory_transactions WHERE vet_id=? AND reason='Govt Dispatch'", (u["id"],)).fetchone()[0]
    vaccinations_given = c.execute("SELECT COALESCE(SUM(dose),0) FROM vaccinations WHERE vet_id=?", (u["id"],)).fetchone()[0]
    resources_sent = 80.0 + dispatched
    resources_left = max(0.0, resources_sent - vaccinations_given)
...
def govt_overview(u,c):
    ...
    for v in c.execute("SELECT * FROM villages ORDER BY name"):
        ...
        outbreak_level = "HIGH" if (confirmed or (max_outbreak is not None and max_outbreak >= 0.85)) else \
                         "MID" if (max_outbreak is not None and max_outbreak >= 0.5) or (3 <= len(active) <= 5) else \
                         "LOW" if len(active) > 0 else None
        village_rows.append({
    ...
    for v in c.execute("SELECT id,name,phone,address FROM users WHERE role='vet' ORDER BY name"):
        ...
        dispatched = c.execute("SELECT COALESCE(SUM(quantity_change),0) FROM inventory_transactions WHERE vet_id=? AND reason='Govt Dispatch'", (v["id"],)).fetchone()[0]
        vaccinations_given = c.execute("SELECT COALESCE(SUM(dose),0) FROM vaccinations WHERE vet_id=?", (v["id"],)).fetchone()[0]
        resources_sent = 80.0 + dispatched
        resources_left = max(0.0, resources_sent - vaccinations_given)
